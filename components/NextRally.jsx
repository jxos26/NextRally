...
const SEED = [
  ['Rico Alvarez', 'High', 0, 'Men'],
  ['Mika Tan', 'High', 0, 'Women'],
  ['Josh Delos Reyes', 'Mid', 0, 'Men'],
  ['Anna Bautista', 'Mid', 0, 'Women'],
  ['Paolo Cruz', 'Low', 0, 'Men'],
  ['Leah Santos', 'Mid', 0, 'Women'],
  ['Denver Yu', 'High', 0, 'Men'],
  ['Carla Mendoza', 'Low', 0, 'Women'],
  ['Bryan Lim', 'Mid', 0, 'Men'],
  ['Trisha Ong', 'High', 0, 'Women'],
  ['Kevin Ramos', 'Low', 0, 'Men'],
  ['Faye Navarro', 'Mid', 1, 'Women'],
  ['Marco Villar', 'High', 1, 'Men'],
  ['Ivy Reyes', 'Low', 0, 'Women'],
]
...
const dayLabel = (key) => new Date(key + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
...
const MODE_HINT = {
  ...
  balanced: 'Fills a court from the deepest single level first; if no level has four waiting, it takes the four closest levels and splits them evenly across the net.',
  manual: 'Nothing auto-fills. Drag names from the waiting list onto the slots, tap a filled slot to send that player back.',
}
...
export default function NextRally() {
  ...
  const [clanFund, setClanFund] = useState('20')
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (d.clanFund) setClanFund(d.clanFund)
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(STORE, JSON.stringify({ players, courts, queue, history, sessions, sessionDate, hours, shuttles, shuttlePrice, guestFee, clanFund, paid, mode }))
    } catch (e) {
      /* storage full or blocked */
    }
  }, [players, courts, queue, history, sessions, sessionDate, hours, shuttles, shuttlePrice, guestFee, clanFund, paid, mode])
  ...
  function endGame(courtId) {
    ...
    const sA = num(c.scoreA),
      sB = num(c.scoreB)
    if (sA === sB) return
    const winTeam = sA > sB ? 0 : 1
    const A = c.slots.slice(0, 2),
      B = c.slots.slice(2, 4)
    const winners = winTeam === 0 ? A : B,
      losers = winTeam === 0 ? B : A
    const mate = {}
    ;[A, B].forEach((t) => {
      if (t[0] && t[1]) {
        mate[t[0]] = t[1]
    ...
    setHistory([
      {
        id: 'g' + Date.now(),
        court: c.name,
        teamA: A.map(nameOf).join(' & '),
        teamB: B.map(nameOf).join(' & '),
        winner: winTeam === 0 ? 'Left' : 'Right',
        winTeam,
        score: sA + ' \u2013 ' + sB,
  ...
  const clan = num(clanFund)
  const share = members.length ? Math.max(0, total - guestTotal) / members.length : 0
  const perMember = share + clan
  const clanTotal = clan * members.length
  const due = (p) => (p.guest ? p.games * fee : perMember)
  const owing = players.filter((p) => p.active && (!p.guest || p.games > 0))
  const collected = owing.filter((p) => paid[p.id]).reduce((t, p) => t + due(p), 0)
  const expected = owing.reduce((t, p) => t + due(p), 0)
  const unpaid = owing.filter((p) => !paid[p.id])
  const allPaid = unpaid.length === 0 && owing.length > 0
  ...
  function removePlayer(p) {
    if (p.games > 0) return
    if (!confirm('Remove ' + p.name + ' from the roster? Their lifetime record goes with them.')) return
  ...
  const rateSpan = !rates.length ? peso(0) : Math.min(...rates) === Math.max(...rates) ? peso(rates[0]) : peso(Math.min(...rates)) + '\u2013' + peso(Math.max(...rates))
  ...
  const tally = (p) => (statsView === 'alltime' ? { g: (p.life?.games || 0) + p.games, w: (p.life?.wins || 0) + p.wins, l: (p.life?.losses || 0) + p.losses } : { g: p.games, w: p.wins, l: p.losses })
    ...
    return (
      <div
        ...
        onDrop={(e) => {
          e.preventDefault()
          e.stopPropagation()
          place(c.id, i, e.dataTransfer.getData('text/plain'))
        }}
      ...
      >
        ...
        <div className='text-[11px] font-bold uppercase tracking-[1px] mt-0.5' style={{ color: p ? chip(p.level).fg : '#7F9389' }}>
      ...
      <aside className='bg-panel border-r border-line px-3.5 py-5 flex flex-col gap-6 sticky top-0 h-screen'>
        ...
        <nav className='flex flex-col gap-0.5'>
          {NAV.map(([id, label]) => {
            const on = screen === id
            return (
              <button key={id} onClick={() => setScreen(id)} className='flex items-center justify-between gap-2 text-left px-3 py-2.5 rounded-[10px] text-sm font-semibold' style={{ background: on ? '#C9F24A' : 'transparent', color: on ? '#0C1210' : '#9FB3A8' }}>
                <span>{label}</span>
            ...
            <div className='min-w-0'>
              ...
              <div className='grid gap-4' style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(310px,1fr))' }}>
                {courts.map((c) => {
                  ...
                  const status = playing ? Math.floor(sec / 60) + ':' + String(sec % 60).padStart(2, '0') : filled === 4 ? 'Ready' : filled ? filled + ' / 4' : 'Open'
                  ...
                  return (
                    <div
                      ...
                      onDrop={(e) => {
                        e.preventDefault()
                        place(c.id, null, e.dataTransfer.getData('text/plain'))
                      }}
                      ...
                      {playing && (
                        <div className='mt-3 grid gap-1.5 items-center' style={{ gridTemplateColumns: 'minmax(0,1fr) 22px minmax(0,1fr)' }}>
                          <div className='flex items-stretch gap-1.5'>
                            <input value={c.scoreA} onChange={(e) => setScore(c.id, 0, e.target.value)} placeholder='0' inputMode='numeric' className='min-w-0 flex-1 p-2 rounded-[9px] border border-slotline bg-slot text-[22px] font-bold text-center tnum' style={{ color: num(c.scoreA) > num(c.scoreB) ? '#C9F24A' : '#F2F7F4' }} />
                            <button onClick={() => bumpScore(c.id, 0, 1)} className='w-[34px] border border-slotline rounded-[9px] bg-slot text-soft hover:text-lime text-base font-bold'>
                              +
                            </button>
                          </div>
                          <div className='grid place-items-center text-[13px] text-[#4C5F57]'>&ndash;</div>
                          <div className='flex items-stretch gap-1.5'>
                            <input value={c.scoreB} onChange={(e) => setScore(c.id, 1, e.target.value)} placeholder='0' inputMode='numeric' className='min-w-0 flex-1 p-2 rounded-[9px] border border-slotline bg-slot text-[22px] font-bold text-center tnum' style={{ color: num(c.scoreB) > num(c.scoreA) ? '#C9F24A' : '#F2F7F4' }} />
                            <button onClick={() => bumpScore(c.id, 1, 1)} className='w-[34px] border border-slotline rounded-[9px] bg-slot text-soft hover:text-lime text-base font-bold'>
                              +
                            </button>
                          </div>
                        </div>
            ...
            <div className='border border-line rounded-2xl bg-panel p-4 min-w-0'>
              ...
              <div className='mt-3.5 flex flex-col gap-1.5 max-h-[60vh] overflow-auto'>
                {queue.map((id, i) => {
                  ...
                  if (!p) return null
                  return (
                    <div key={id} draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', id)} className='flex items-center gap-2.5 px-2.5 py-2 rounded-[10px] bg-panel2 border border-[#1F2E28] cursor-grab'>
          ...
          <div className='grid gap-5 items-start' style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))' }}>
            <div className='border border-line rounded-2xl bg-panel overflow-x-auto min-w-0'>
              <div className='min-w-[760px]'>
                <div className='grid gap-2.5 px-4 py-3 border-b border-line text-[11px] tracking-[1.2px] uppercase text-mute' style={{ gridTemplateColumns: 'minmax(140px,2fr) 74px 76px 76px 86px 172px' }}>
                  <div>Player</div>
                  <div>Level</div>
                  <div>Div</div>
                  <div>Type</div>
                ...
                {players.map((p) => {
                  ...
                  return (
                    <div key={p.id} className='grid gap-2.5 px-4 py-3 border-b border-[#16211D] items-center' style={{ gridTemplateColumns: 'minmax(140px,2fr) 74px 76px 76px 86px 172px' }}>
                      <div className='min-w-0'>
                      ...
                      <div className='text-[13px]' style={{ color: p.guest ? '#D9CE84' : '#9FB3A8' }}>
                        {p.guest ? 'Guest' : 'Member'}
                      </div>
                      <div className='text-[13px]' style={{ color: !p.active ? '#5A6B63' : onCourtNow ? '#C9F24A' : '#9FB3A8' }}>
                        {!p.active ? 'Out' : onCourtNow ? 'On court' : 'Waiting'}
                      </div>
                      <div className='justify-self-end flex gap-1.5'>
                        <button onClick={() => togglePlayer(p)} className='w-[82px] px-2.5 py-1.5 border border-line3 rounded-lg text-soft hover:text-chalk text-xs'>
                          {p.active ? 'Check out' : 'Check in'}
                        </button>
                        <button
                          onClick={() => removePlayer(p)}
                          disabled={p.games > 0}
                          title={p.games > 0 ? 'Played tonight \u2014 check out instead, or remove after the session closes.' : 'Remove ' + p.name + ' from the roster.'}
                          className='w-[74px] px-2.5 py-1.5 border rounded-lg text-xs disabled:cursor-not-allowed'
                          style={{
                            borderColor: p.games > 0 ? '#232E2B' : '#3A2A2A',
                            color: p.games > 0 ? '#5A6B63' : '#C98A8A',
                          }}
                        >
                          Remove
                        </button>
              ...
              <div className='mt-3.5 flex flex-col gap-2.5'>
                ...
                <div className='flex gap-1.5'>
                  {ORDER.map((lv) => {
                    ...
                    return (
                      <button key={lv} onClick={() => setNewLevel(lv)} className='flex-1 py-2.5 rounded-[10px] border text-[13px] font-bold' style={{ background: on ? chip(lv).bg : 'transparent', color: on ? chip(lv).fg : '#7F9389', borderColor: on ? chip(lv).fg : '#24352E' }}>
                        {lv}
                      </button>
                    )
                  })}
                ...
                <div className='flex gap-1.5'>
                  {['Men', 'Women'].map((g) => {
                    ...
                    return (
                      <button key={g} onClick={() => setNewGender(g)} className='flex-1 py-2.5 rounded-[10px] border text-[13px] font-semibold' style={{ background: on ? '#22332C' : 'transparent', color: on ? '#DCEFD9' : '#7F9389', borderColor: on ? '#3E534A' : '#24352E' }}>
                        {g}
                      </button>
                    )
                  })}
                ...
                <div className='flex gap-1.5'>
                  {[
                    ...
                    return (
                      <button key={label} onClick={() => setNewGuest(val)} className='flex-1 py-2.5 rounded-[10px] border text-[13px] font-semibold' style={{ background: on ? '#22332C' : 'transparent', color: on ? '#DCEFD9' : '#7F9389', borderColor: on ? '#3E534A' : '#24352E' }}>
                        {label}
                      </button>
                    )
                  })}
                </div>
                <button onClick={addPlayer} className='mt-1 py-3 rounded-[10px] bg-lime text-ink text-sm font-bold'>
                  Add to waiting list
                </button>
              </div>
              <p className='mt-4 text-xs text-dim leading-relaxed'>Guests pay {peso(fee)} per game and are left out of the session split.</p>
            </div>
          </div>
        )}
          ...
          <div className='grid gap-5 items-start' style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))' }}>
              ...
              {courts.map((c) => (
                <div key={c.id} className='border border-line rounded-2xl bg-panel p-4'>
                  ...
                  <div className='mt-3 flex gap-2'>
                    <button onClick={() => setCourts(courts.map((x) => (x.id === c.id ? { ...x, rate: Math.max(0, x.rate - 50) } : x)))} className='flex-1 py-2 border border-line3 rounded-[9px] text-soft text-sm'>
                      &minus;50
                    </button>
                    <button onClick={() => setCourts(courts.map((x) => (x.id === c.id ? { ...x, rate: x.rate + 50 } : x)))} className='flex-1 py-2 border border-line3 rounded-[9px] text-soft text-sm'>
                      +50
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      setCourts(courts.filter((x) => x.id !== c.id))
                      setQueue(queue.concat(c.slots.filter(Boolean)))
                    }}
            ...
            <div className='border border-line rounded-2xl bg-panel p-4'>
              ...
              <div className='mt-3.5 flex flex-col gap-2.5'>
                ...
                <button onClick={addCourt} className='py-3 rounded-[10px] bg-lime text-ink text-sm font-bold'>
                  Add court
                </button>
              </div>
            </div>
          ...
          <>
            <div className='flex flex-wrap gap-2 items-center mb-4'>
              ...
              {[
                ...
                return (
                  <button key={id} onClick={() => setStatsView(id)} className='px-3.5 py-2 rounded-full border text-[13px] font-semibold' style={{ background: on ? '#C9F24A' : '#141F1A', color: on ? '#0C1210' : '#9FB3A8', borderColor: on ? '#C9F24A' : '#24352E' }}>
                    {label}
                  </button>
                )
              })}
            ...
            <div className='border border-line rounded-2xl bg-panel overflow-x-auto'>
              <div className='min-w-[760px]'>
                <div className='grid gap-2.5 px-4 py-3 border-b border-line text-[11px] tracking-[1.2px] uppercase text-mute' style={{ gridTemplateColumns: 'minmax(0,1.6fr) 70px 60px 60px 90px minmax(0,1.4fr) 100px' }}>
                  ...
                  <div>L</div>
                  <div>Win rate</div>
                  <div>Most played with</div>
                  <div className='text-right'>{statsView === 'alltime' ? 'Tonight' : 'Owes'}</div>
                </div>
                {players
                  .slice()
                  .sort((a, b) => {
                  ...
                  .map((p) => {
                    ...
                    return (
                      <div key={p.id} className='grid gap-2.5 px-4 py-3 border-b border-[#16211D] items-center' style={{ gridTemplateColumns: 'minmax(0,1.6fr) 70px 60px 60px 90px minmax(0,1.4fr) 100px' }}>
                        <div className='min-w-0'>
                          <div className='text-[15px] font-semibold truncate'>{p.name}</div>
                          <div className='text-[11px] font-bold tracking-[0.8px] uppercase mt-0.5' style={{ color: chip(p.level).fg }}>
                            {p.level}
                          </div>
                        </div>
                        ...
                        <div className='text-right text-[15px] font-bold tnum'>{statsView === 'alltime' ? (p.games ? p.games + ' tonight' : '\u2014') : !p.active ? '\u2014' : p.guest ? peso(p.games * fee) : peso(perMember)}</div>
                      </div>
                    )
                  })}
              </div>
            ...
            <div className='border border-line rounded-2xl bg-panel p-4'>
              ...
              <div className='mt-3.5 flex flex-col gap-3.5'>
                {[
                  ...
                  ['Clan fund per member', clanFund, setClanFund],
                ].map(([label, val, set]) => (
                  <div key={label}>
                    ...
                    {label === 'Clan fund per member' && <div className='text-xs text-dim mt-1.5 leading-relaxed'>Added on top of each member&rsquo;s share. Guests don&rsquo;t pay it.</div>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className='min-w-0 flex flex-col gap-4'>
              <div className='grid gap-3' style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))' }}>
                {[
                  ...
                  { label: 'Clan fund', value: '+' + peso(clanTotal), sub: `${peso(clan)} \u00D7 ${members.length} members`, blue: true },
                  { label: 'Each member pays', value: peso(perMember), sub: `${peso(share)} share + ${peso(clan)} fund`, hi: true },
                ].map((t) => (
                  <div key={t.label} className='rounded-[14px] border p-[15px]' style={{ background: t.hi ? '#1B2A12' : '#0F1714', borderColor: t.hi ? '#3F5C1E' : '#1C2823' }}>
                    <div className='text-[11px] tracking-[1.3px] uppercase' style={{ color: t.hi ? '#B7CE86' : '#7F9389' }}>
                      {t.label}
                    </div>
                    <div className='text-[25px] font-bold mt-2 tnum tracking-[-0.5px]' style={{ color: t.hi ? '#C9F24A' : t.sand ? '#D9CE84' : t.blue ? '#8FBEDA' : '#F2F7F4' }}>
                      {t.value}
                    </div>
                    <div className='text-xs mt-1' style={{ color: t.hi ? '#9FB37A' : '#5A6B63' }}>
                      {t.sub}
                    </div>
                  </div>
                ))}
              ...
              <div className='border border-line rounded-2xl bg-panel p-4'>
                ...
                <div className='mt-2.5 text-[15px] leading-relaxed text-[#DCEFD9] tnum text-pretty'>
                  ({courts.length} courts &times; {hours}h at {rateSpan} = {peso(courtCost)}) + ({shuttles} shuttles &times; {peso(num(shuttlePrice))}) &minus; {peso(guestTotal)} guest fees = {peso(Math.max(0, total - guestTotal))} &divide; {members.length} members = {peso(share)} each, + {peso(clan)} clan fund = {peso(perMember)} each
                </div>
                <p className='mt-2 text-[13px] text-mute leading-relaxed text-pretty'>Guest games are collected first and taken off the top, so members only split what&rsquo;s left.</p>
              </div>

              <div className='border border-line rounded-2xl bg-panel overflow-x-auto'>
                <div className='min-w-[560px]'>
                  <div className='flex flex-wrap gap-3 justify-between items-center px-4 py-4 border-b border-line'>
                    <div className='min-w-0'>
                      <div className='text-[15px] font-bold tnum'>
                        {peso(collected)} of {peso(expected)} collected
                      </div>
                      <div className='text-xs mt-1' style={{ color: allPaid ? '#C9F24A' : '#D9CE84' }}>
                        {!owing.length ? 'Nobody owes anything yet' : allPaid ? `All ${owing.length} collected` : `${unpaid.length} still to collect`}
                      </div>
                      <div className='h-1 rounded-full bg-line mt-2 overflow-hidden w-[200px] max-w-full'>
                        <div className='h-full bg-lime' style={{ width: (expected ? Math.round((collected / expected) * 100) : 0) + '%' }} />
                      </div>
                    </div>
                    <button onClick={markAll} className='px-3.5 py-2.5 border border-line3 rounded-[10px] text-soft hover:text-chalk text-[13px] font-semibold'>
                      {allPaid ? 'Clear all' : 'Mark all paid'}
                    </button>
                  </div>
                  <div className='grid gap-2.5 px-4 py-3 border-b border-line text-[11px] tracking-[1.2px] uppercase text-mute' style={{ gridTemplateColumns: 'minmax(140px,1fr) 84px 70px 96px 104px' }}>
                    <div>Who</div>
                    <div>Type</div>
                    <div>Games</div>
                    <div className='text-right'>Pays</div>
                  ...
                  {players
                    ...
                    .map((p) => {
                      ...
                      return (
                        <div key={p.id} className='grid gap-2.5 px-4 py-3 border-b border-[#16211D] items-center' style={{ gridTemplateColumns: 'minmax(140px,1fr) 84px 70px 96px 104px' }}>
                          <div className='text-[15px] font-semibold truncate'>{p.name}</div>
                          <div className='text-[13px]' style={{ color: p.guest ? '#D9CE84' : '#9FB3A8' }}>
                            {p.guest ? 'Guest' : 'Member'}
                          </div>
                          <div className='text-sm tnum text-soft'>{p.games}</div>
                          <div className='text-right text-[15px] font-bold tnum' style={{ color: isPaid ? '#7F9389' : '#F2F7F4' }}>
                            {peso(due(p))}
                          </div>
                          <button
                            onClick={() => markPaid(p.id)}
                            className='justify-self-end px-3 py-[7px] rounded-[9px] border text-xs font-bold'
                            style={{
                              background: isPaid ? '#1B2A12' : 'transparent',
                              color: isPaid ? '#C9F24A' : '#9FB3A8',
                              borderColor: isPaid ? '#3F5C1E' : '#2A3B34',
                            }}
                          >
          ...
          <>
            <div className='flex flex-wrap gap-2 items-center mb-4'>
              {[
                ...
                ['past', 'Past sessions' + (sessions.length ? ' (' + sessions.length + ')' : '')],
              ].map(([id, label]) => {
                const on = historyView === id
                return (
                  <button key={id} onClick={() => setHistoryView(id)} className='px-3.5 py-2 rounded-full border text-[13px] font-semibold' style={{ background: on ? '#C9F24A' : '#141F1A', color: on ? '#0C1210' : '#9FB3A8', borderColor: on ? '#C9F24A' : '#24352E' }}>
                    {label}
                  </button>
                )
              })}
                ...
                {history.map((h) => (
                  <div key={h.id} className='grid gap-3.5 px-4 py-4 border-b border-[#16211D] items-center' style={{ gridTemplateColumns: '96px minmax(0,1fr) 120px' }}>
                    ...
                    <div className='min-w-0 text-sm leading-relaxed text-pretty'>
                      <span className='font-semibold' style={{ color: h.winTeam === 0 ? '#F2F7F4' : '#6E8279' }}>
                        {h.teamA}
                      </span>
                      <span className='font-bold px-1.5 text-[#4C5F57]'>vs</span>
                      <span className='font-semibold' style={{ color: h.winTeam === 1 ? '#F2F7F4' : '#6E8279' }}>
                        {h.teamB}
                      </span>
                    </div>
                    <div className='text-right'>
                      ...
                      <div className='text-xs text-mute tnum'>
                        {h.score ? h.winner + ' won \u00B7 ' : ''}
                        {h.duration}
                      </div>
                    </div>
                  </div>
                ))}
                {!history.length && <div className='px-4 py-8 text-center text-sm text-dim'>No games finished yet. End a game on the live board and it lands here.</div>}
              </div>
            )}
            ...
            {historyView === 'past' && (
              <div className='flex flex-col gap-3.5'>
                ...
                {!sessions.length && <div className='border border-line rounded-2xl bg-panel px-4 py-8 text-center text-sm text-dim'>No closed sessions yet. Use &ldquo;Close out session&rdquo; when the night ends.</div>}
              </div>
            )}
          </>
        )}
      ...
      {closePrompt && (
        <div className='fixed inset-0 grid place-items-center p-6 z-50' style={{ background: 'rgba(6,10,9,0.74)' }}>
          <div className='w-full max-w-[460px] rounded-[18px] border border-[#2F5C49] bg-panel p-[22px]'>
            ...
            <div className='text-[22px] font-bold mt-2 tracking-[-0.4px] text-pretty'>{!hasPlay ? 'Nothing to close yet.' : sessionDate === dayKey(new Date()) ? 'Close out ' + dayLabel(sessionDate) + '?' : dayLabel(sessionDate) + ' is still open.'}</div>
            <p className='mt-2.5 text-sm leading-relaxed text-soft text-pretty'>
              {hasPlay ? (
                <>
                  {history.length} games &middot; {peso(total)} court and shuttles &middot; {peso(expected)} to collect including the clan fund. Closing files this session under {dayLabel(sessionDate)}, folds tonight&rsquo;s record into every player&rsquo;s lifetime totals, and opens a clean board for {dayLabel(dayKey(new Date()))}. Payment resets to zero.
                </>
              ) : (
                <>No games have finished in this session, so there is nothing to file. Closing just rolls the board over to {dayLabel(dayKey(new Date()))} &mdash; no session record is written and no payment is recorded.</>
              )}
            </p>
            {hasPlay && unpaid.length > 0 && (
              <div className='mt-3.5 px-3.5 py-3 rounded-[11px] border border-[#4A431E] bg-[#221F12]'>
                <div className='text-xs tracking-[1.2px] uppercase text-sand'>Still to collect</div>
                <div className='text-[13px] text-[#DCD6AE] mt-1.5 leading-relaxed text-pretty'>
                  {unpaid.length} of {owing.length} haven&rsquo;t paid yet:{' '}
                  {unpaid
                    .slice(0, 4)
                    .map((p) => p.name)
                    .join(', ')}
                  {unpaid.length > 4 ? ' +' + (unpaid.length - 4) + ' more' : ''}
                </div>
              </div>
            )}
